...
`+pdf_h+` <small class="units">`+display_unit+`</small>
 ...
 `+pdf_w+` <small class="units">`+display_unit+`</small>
  ...
  _getPdfPageForPageNumber: function shark(pageNum) {
    ...
    console.log(" _getPdfPageForPageNumber ["+pageNum+"] -> "+reversePageNum+" :: "+is_interlaced)
    pageNum = reversePageNum
    /** @return -- either the page number if still searching OR [upload block index, PDFPage] if found */
    ...
    if (is_interlaced) {
      ...
      return [block_i, block.pdfDoc.getPage(pageNum)]
    }
